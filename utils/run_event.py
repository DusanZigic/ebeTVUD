...
if __name__ == "__main__":
	...
	if params['freestream']['turn_on'] == 1:
		call('python3 stream_ic.py', shell=True, cwd=cwdir, stdout=runrecord, stderr=errorrecord)
		...
		remove(path.join(cwdir, 'stream_ic.py'))
	...
	#particlization
	call('python3 sample_surface.py', shell=True, cwd=cwdir, stdout=runrecord, stderr=errorrecord)
	...
	for aFile in rm_files: remove(path.join(cwdir, aFile))
	remove(path.join(cwdir, 'sample_surface.py'))
	...
	with open(path.abspath('eventdone.info'), 'w') as f: f.write('event done\n')